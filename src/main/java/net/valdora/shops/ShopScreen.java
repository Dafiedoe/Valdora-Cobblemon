...
public class ShopScreen extends Screen {
    ...
    protected void init() {
        ...
        if (showPurchaseOverlay && selectedItem != null) {
            ...
            this.addDrawableChild(ButtonWidget.builder(Text.literal("-"), b -> adjustAmount(-1)).dimensions(x + 16, y + 36, 20, 20).build());
            
            this.addDrawableChild(ButtonWidget.builder(Text.literal("+"), b -> adjustAmount(1)).dimensions(x + panelWidth - 36, y + 36, 20, 20).build());
            
            this.addDrawableChild(ButtonWidget.builder(Text.literal("Cancel"), b -> closePurchaseOverlay()).dimensions(x + 16, y + panelHeight - 30, 60, 20).build());
            
            this.addDrawableChild(ButtonWidget.builder(Text.literal("Buy"), b -> confirmPurchase()).dimensions(x + panelWidth - 76, y + panelHeight - 30, 60, 20).build());
        } else {
            this.addDrawableChild(ButtonWidget.builder(Text.literal("Close"), (b) -> this.client.player.closeHandledScreen()).dimensions(this.width - 80, this.height - 30, 70, 20).build());
        }
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawText(this.textRenderer, balance, this.width - balanceWidth - 10, 10, 0x55FF55, false);
        ...
        if (showPurchaseOverlay && selectedItem != null) {
            ...
            context.drawText(this.textRenderer, Text.literal(amt), x + panelWidth / 2 - this.textRenderer.getWidth(amt) / 2, y + 42, 0x000000, false);
            
            String total = "Total: " + (selectedItem.cost * purchaseAmount);
            context.drawText(this.textRenderer, Text.literal(total), x + panelWidth / 2 - this.textRenderer.getWidth(total) / 2, y + 60, 0xAA0000, false);
        }
