...
import net.valdora.pokephone.PokePhoneInitializer;
...
public class Valdora implements ModInitializer {
    ...
    public static void loadConfig() {
        ...
        if (!configFile.exists()) {
            ...
            LOGGER.info("[Valdora] Default world spawn: {} @ ({}, {}, {}) yaw/pitch: {}/{}", WORLD_SPAWN_DIMENSION, WORLD_SPAWN_X, WORLD_SPAWN_Y, WORLD_SPAWN_Z, WORLD_SPAWN_YAW, WORLD_SPAWN_PITCH);
            return;
        }
        ...
        try (FileReader reader = new FileReader(configFile)) {
            ...
            if (cfg.has("min_ticks_before_encounter")) {
                TallGrassWalkEvent.MIN_TICKS_BEFORE_ENCOUNTER = cfg.get("min_ticks_before_encounter").getAsInt();
            }
            if (cfg.has("encounter_chance_per_tick")) {
                TallGrassWalkEvent.ENCOUNTER_CHANCE_PER_TICK = cfg.get("encounter_chance_per_tick").getAsDouble();
            }
            ...
            if (cfg.has("world_spawn") && cfg.get("world_spawn").isJsonObject()) {
                ...
                try {
                    ...
                    LOGGER.info("[Valdora] Loaded world spawn from config: {} @ ({}, {}, {}) yaw/pitch: {}/{}", WORLD_SPAWN_DIMENSION, WORLD_SPAWN_X, WORLD_SPAWN_Y, WORLD_SPAWN_Z, WORLD_SPAWN_YAW, WORLD_SPAWN_PITCH);
                } catch (Exception e) {
                    LOGGER.warn("[Valdora] Invalid world_spawn entry in config.json; using defaults", e);
                }
            } else {
                LOGGER.info("[Valdora] Using default world spawn: {} @ ({}, {}, {}) yaw/pitch: {}/{}", WORLD_SPAWN_DIMENSION, WORLD_SPAWN_X, WORLD_SPAWN_Y, WORLD_SPAWN_Z, WORLD_SPAWN_YAW, WORLD_SPAWN_PITCH);
            }
